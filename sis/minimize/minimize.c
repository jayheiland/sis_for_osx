...

{
	...
	foreachi_set(A, i, p) {
		...
		(void) set_copy(restrict1, p);
		min_set_ord = SIZE(p);
	    } else if (SIZE(p) == min_set_ord) {
		(void) set_or(restrict1, restrict1, p);
	    }
	}
	...
	/* Check for "essential" columns */
		...
		Abar = ncp_unate_complement(ncp_abs_covered_many(A, restrict1),
							    max_lit-i);
		sf_free(A);
		foreachi_set(Abar, i, p) {
		    (void) set_or(p, p, restrict1);
		}
	    }

	/* else, recur as usual */
	} else {
	    max_i = ncp_abs_select_restricted(A, restrict1);
 ...
 */
static int
ncp_abs_select_restricted(A, restrict1)
pset_family A;
pset restrict1;
{
